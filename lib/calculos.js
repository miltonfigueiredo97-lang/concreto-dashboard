...
export function calcIndicePerda(lancamentos, btsConfig) {
  ...
  // Índice: média de (previsto-executado)/previsto por BT
  const indice = btsLancadas.reduce((s,b,_,arr) => {
    const d = detalhes.find(d => d.bt.id === b.id);
    return s + ((b.volumePrevisto - d.usado) / b.volumePrevisto * 100);
  }, 0) / btsLancadas.length;
...
export function calcKPIs(pecas, lancamentos, btsConfig, filtroAndar='todos') {
  ...
  return { totalVol, concVol, projFaltando, realFaltando, pctConc,
    volPrevisto:prev.total, volPrevistoFaltando:prev.faltando, perdaInfo };
}
