...
export function calcKPIs(pecas, lancamentos, btsConfig, filtroAndar='todos') {
  ...
  // Volume Executado de Projeto = soma real lançado por peça, limitado ao volConc
  // Se lançou 100% de uma peça de 0.10m³, conta 0.10m³ (não 0.17m³ de lançamentos antigos)
  const execVol = ps.reduce((s,p)=>s+Math.min(p.volume, volLancadoPeca(p.id,lancamentos)), 0);
