...
export function pctConcretado(peca, lancamentos) {
  ...
  const pct = peca.volume > 0 ? (vc/peca.volume)*100 : 0;
  // Tolerância: diferença menor que 0.005 m³ considera 100%
...
export function calcIndicePerda(lancamentos, btsConfig) {
  ...
  const detalhes = btsLancadas.map(b => {
    ...
    const perdaO= lans.reduce((s,l) => s+(parseFloat(l.perdaObra)||0), 0);
    const perdaC= parseFloat(lans[0]?.perdaCocho)||0; // cocho/linha: único por BT
    ...
    return { bt:b, usado, perdaObra:perdaO, perdaCocho:perdaC, difCaminhao:difCam };
  });

  const perdaCaminhao = totalPrevisto - totalExecutado; // + = perda, - = sobra
  const perdaTotal    = totalPerdaObra + Math.max(0, perdaCaminhao);
  // Índice real: descontando perda de cocho (inevitável) do previsto
  const previstoSemCocho = totalPrevisto - totalPerdaCocho;
  const indice = previstoSemCocho>0 ? (Math.max(0, previstoSemCocho - totalExecutado) / previstoSemCocho) * 100 : 0;

  return { indice, perdaTotal, perdaCaminhao, perdaObra:totalPerdaObra, perdaCocho:totalPerdaCocho, totalPrevisto, totalExecutado, detalhes };
}

export function calcKPIs(pecas, lancamentos, btsConfig, filtroAndar='todos', pecasOrig) {
  const ps = filtroAndar==='todos' ? pecas : pecas.filter(p=>p.andar===filtroAndar);
  const totalVol = ps.reduce((s,p)=>s+p.volume, 0);
  // Volume Real Concretado = soma dos volumePrevisto das BTs que foram lançadas
  const btIdsLan = new Set(lancamentos.map(l=>l.btConfigId));
  const btsLan = btsConfig.filter(b=>btIdsLan.has(b.id));
  const concVol = btsLan.reduce((s,b)=>s+b.volumePrevisto, 0);
  // Volume Executado = lançado por peça, limitado ao volume ORIGINAL da peça (projeto)
  // pecasOrig tem os volumes reais do projeto sem ajuste de pctConcretagem
  const psOrig = pecasOrig || pecas;
  const execVol = ps.reduce((s,p)=>{
    const orig = psOrig.find(o=>o.id===p.id);
    const limiteVol = orig ? orig.volume : p.volume;
    return s + Math.min(limiteVol, volLancadoPeca(p.id,lancamentos));
  }, 0);
  ...
  // Volume real faltando = projeto - executado real
  ...
  return { totalVol, concVol, execVol, projFaltando, realFaltando, pctConc,
...
export function calcPorTipo(pecas, lancamentos) {
  ...
  return tipos.map(t => {
    ...
    const prog = ps.reduce((s,p)=>s+p.volume, 0);
    const conc = ps.reduce((s,p)=>s+Math.min(p.volume,volLancadoPeca(p.id,lancamentos)), 0);
    const faltRaw = prog - conc;
    const falt = faltRaw < 0.005 ? 0 : Math.max(0, faltRaw);
    const pct  = prog>0?(conc/prog)*100:0;
