/**
 * lib/sheets.js
 * Lê dados do Google Sheets via API REST pública.
 *
 * ESTRUTURA DAS ABAS:
 *
 * Aba "Pecas":
 ...
 *
 * Aba "BTs":
 *   A:id | B:concretagemId | C:numero | D:volumeCaminhao | E:sobraCaminhao | F:perdaObra | G:hora | H:obs
 *
 * Aba "Lancamentos":
 *   A:id | B:btId | C:concretagemId | D:pecaId | E:pct | F:volume | G:hora
 */
...
export async function getBTs() {
  const rows = await getRange('BTs!A:H');
  return skipHeader(rows).map(r => ({
    id:             r[0] || '',
    concretagemId:  r[1] || '',
    numero:         parseInt(r[2])   || 0,
    volumeCaminhao: parseFloat(r[3]) || 0,
    sobraCaminhao:  parseFloat(r[4]) || 0,
    perdaObra:      parseFloat(r[5]) || 0,
    hora:           r[6] || '',
    obs:            r[7] || '',
  })).filter(b => b.id);
}
...
export async function getAllData() {
  const [pecas, concretagens, bts, lancamentos] = await Promise.all([
    getPecas(), getConcretagens(), getBTs(), getLancamentos(),
  ]);
  return { pecas, concretagens, bts, lancamentos };
}
