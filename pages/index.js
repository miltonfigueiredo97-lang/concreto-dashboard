                  ...
                  :pecasVisiveis.map(p=>{const sel=!!vinculos.find(v=>v.pecaId===p.id);const vinc=vinculos.find(v=>v.pecaId===p.id);return(
                    <div key={p.id} style={{display:'flex',alignItems:'center',gap:12,padding:'12px 16px',borderBottom:'1px solid var(--border)',background:sel?'rgba(232,162,37,0.06)':'transparent'}}>
                      ...
                      {sel&&<div style={{display:'flex',alignItems:'center',gap:8,flexShrink:0}}>
                        ...
                        <input type="text" inputMode="numeric" value={vinc.pctConcretagem} onChange={e=>{const v=e.target.value.replace(/[^0-9]/g,'');setPct(p.id,v===''?'':v);}} onBlur={e=>{const ja=pecaConc.filter(pc=>pc.pecaId===p.id&&pc.concretagemId!==concId).reduce((s,pc)=>s+parseFloat(pc.pctConcretagem||0),0);const maxVal=Math.max(1,100-ja);const raw=parseFloat(e.target.value);const v=isNaN(raw)||raw<1?1:Math.min(raw,maxVal);setPct(p.id,v);}} onClick={e=>e.stopPropagation()} style={{width:64,background:'var(--surface2)',border:'1px solid var(--accent)',color:'var(--accent)',fontFamily:'var(--mono)',fontSize:13,padding:'6px 8px',outline:'none'}}/>
      ...
      <div className={s.mainContent}>
        ...
        {tab==='operacional'&&(
          <main className={`${s.page} animate-fadein`}>
            ...
            {/* EXPORTAR PEÇAS POR CONCRETAGEM */}
            <div style={{display:'flex',justifyContent:'flex-end',marginBottom:8}}>
              <button onClick={()=>{
                const rows=[['Nome','Tipo','Andar','Volume Projeto (m3)','Concretagem','% Nesta Conc','Vol. Nesta Conc. (m3)']];
                const concsMap=Object.fromEntries(concretagens.map(c=>[c.id,`N${c.numero} - ${c.data}${c.descricao?' | '+c.descricao:''}`]));
...
');
                ...
                a.click();
              }} style={{background:'var(--surface2)',border:'1px solid var(--border)',color:'var(--text2)',fontSize:12,padding:'6px 14px',borderRadius:'var(--radius-sm)',cursor:'pointer',display:'flex',alignItems:'center',gap:6}}>
                📥 Exportar Peças por Concretagem
              </button>
            </div>
            ...
            <div className={s.grid2}>
              ...
              <div className={s.panel}>
                ...
                <GraficoTipos pecas={filtroConc!=='todas'?pecasParaKPI.filter(p=>(filtroAndar==='todos'||p.andar===filtroAndar)&&(filtroTipoOp==='todos'||p.tipo===filtroTipoOp)):pecasFiltOp} lancamentos={lancamentosOp} btsConfig={btsConfig} concretagens={concretagens} pecaConc={pecaConc}/>
              </div>
